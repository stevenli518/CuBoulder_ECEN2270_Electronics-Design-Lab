...
void Move_two() {
  enc_count_L = 0;
  enc_count_R = 0;
  while (enc_count_L <= 2710 * 2 && enc_count_R <= 2780 * 2) {
...
void Move_one() {
  enc_count_L = 0;
  enc_count_R = 0;
  while (enc_count_L <= 2710 && enc_count_R <= 2780) {
    forward(pinLeftForward, pinRightForward);
  }
  digitalWrite(pinRightForward, LOW);
  digitalWrite(pinLeftForward, LOW);
...
void CW90() {
  enc_count_L = 0;
  enc_count_R = 0;
  while (enc_count_L <= 1000 && enc_count_R <= 900) {
    Rotate90(pinRightForward, pinLeftBackward);
  }
  digitalWrite(pinRightForward, LOW);
  digitalWrite(pinLeftBackward, LOW);
  delay(1000);
...
void CC90() {
  enc_count_L = 0;
  enc_count_R = 0;
  while (enc_count_L <= 900 && enc_count_R <= 1000) {
    Rotate90(pinLeftForward, pinRightBackward);
  }
  digitalWrite(pinLeftForward, LOW);
  digitalWrite(pinRightBackward, LOW);
  delay(1000);
...
void Rotate90(int forward, int backward) {
